...
The Windows v0.1.7 installer is not code-signed. Confirm its SHA-256 digest using:

```powershell
Get-FileHash -Algorithm SHA256 '.\Local Idea Studio-0.1.7-win-x64.exe'
```
...
Agent Mode is disabled by default. The v0.1.7 preview only supports text-file creation, replacement, append, and folder creation inside a user-selected workspace. Every operation is validated in the Electron main process and requires a native **Allow once** confirmation. Shell execution, deletion, absolute paths, path traversal, and linked-folder escapes are rejected.
