...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "local-idea-paths-"));
...
try {
  ...
  assert.equal(defaultRoot, path.join(documentsDir, "Local Idea Studio"));
  ...
  assert.equal(fs.existsSync(path.join(defaultRoot, ".local-idea-root.json")), true);
